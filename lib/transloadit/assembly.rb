...
#
module Transloadit
  class Assembly < Transloadit::ApiModel
    DEFAULT_TRIES = 3
    ...
    def _wrap_steps_in_hash(steps)
      ...
      when Transloadit::Step  then steps.to_hash
        ...
        raise ArgumentError, 'There are different Assembly steps using the same name' if steps.uniq(&:name) != steps
    ...
    def _handle_rate_limit!(response, ios, is_retrying)
      if is_retrying
        ...
        raise Transloadit::Exception::RateLimitReached, response
      end
    end
  end
end
